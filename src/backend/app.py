...
from .services.prices import get_quote, get_intraday_points
...
def add_watch(item: WatchCreate):
    ...
    try:
        ...
        obj = WatchItem(symbol=sym, name=item.name)
        db.add(obj)
        db.commit()
...
def get_ab(symbol: str):
    ...
    try:
        ...
        if not obj:
            ...
            obj = ABSignalCache(symbol=symbol.upper(), suggestion=data.get("suggestion"),
                                last_two_actions=data.get("last_two_actions") or [],
                                summary=data.get("summary"))
            db.add(obj); db.commit(); db.refresh(obj)
        return {
            "symbol": obj.symbol,
            "suggestion": obj.suggestion,
            "last_two_actions": obj.last_two_actions or [],
            "summary": obj.summary,
